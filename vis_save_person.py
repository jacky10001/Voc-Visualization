...
for filename_ in files_name:
    ...
    for i, object in enumerate(objects):
        ...
        if cls_name == "person":
            ...
            # get data   str --> int
            xmin_data = int(eval(xmin.childNodes[0].data))
            ymin_data = int(eval(ymin.childNodes[0].data))
            xmax_data = int(eval(xmax.childNodes[0].data))
            ymax_data = int(eval(ymax.childNodes[0].data))
            ...
            if (abs(roi_w - roi_h) < 30) and (roi_w > 50 and roi_h > 50):
                ...
                flag = cv2.imwrite(save_path, cv2.resize(roi, (70,70)))
                if(flag):
